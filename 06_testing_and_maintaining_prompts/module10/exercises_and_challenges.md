...
**Check your work:**
- [ ] All six cells (v1 and v2 score per ticket) are filled
- [ ] Each ticket's verdict (better/same/worse) matches the direction of its scores
- [ ] The promotion decision is justified from the totals — not from "v2 looks nicer"
...
**Check your work:**
- [ ] (a) is attributed to the change that removed the ambiguous-ticket example (v4), with the reason
- [ ] (b) is attributed to the change that reworded a rule (v3), with the reason
- [ ] The violated five-property (10.4) answer names real properties (e.g., one-change isolation, tracked/rationale), not a paraphrase of the story
...
**Check your work:**
- [ ] Every hypothesis is falsifiable — measurable X changes Y by Z (not "make it clearer" restated)
- [ ] Each has a named metric behind it (rubric average, category-accuracy, parse rate, length)
...
**Check your work:**
- [ ] You correctly say the 6% cannot be attributed — the two changes are confounded
- [ ] Your prescription is specific: revert, re-apply one change at a time, re-measure
...
**Check your work:**
- [ ] Each recommendation is consistent with that team's size, count of prompts/products, and release cadence
- [ ] The demanded registry property is concrete (e.g., audit log, diff history, rollback) and you can justify it for security-sensitive prompts
...
**Check your work:**
- [ ] The template has a blank for a specific, falsifiable hypothesis — not a "fill it however" box
- [ ] The "change" section forces one isolated diff and asks why only that change
- [ ] Testing hooks cover the Module 9 evaluation AND the Module 8 adversarial set
- [ ] The decision gate makes promote / reject / iterate explicit
- [ ] The rollback section names a concrete last-known-good version id
- [ ] A teammate could pick it up and follow it without your explanation
...
**Check your work:**
- [ ] Every blank is filled for a real notional change (no placeholder-looking values)
- [ ] Version ids are concrete and the "last known-good id" is named
- [ ] The decision-gate outcome is an explicit promote / reject / iterate
- [ ] Any blank you could not fill became a fix to the template itself — and you noted it