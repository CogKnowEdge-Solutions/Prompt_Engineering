...
**Check your work:**
- [ ] The completed template has at least one example per output class (High/Medium/Low priority, each department, etc.)
- [ ] Examples are varied enough that they show a pattern, not near-duplicates
- [ ] Every example uses the exact same arrow and label format as the template
- [ ] The test row at the end is left blank for the model to complete
...
**Check your work:**
- [ ] All examples now use consistent formatting (same arrow symbols, same labels)
- [ ] Examples cover all relevant classes — no label bias from a one-sided set
- [ ] Instructions are clearly separated from content with delimiters or tags where needed
- [ ] The edge case from the original (e.g., refund/return ticket) is now covered or split out
- [ ] You re-ran at least one fixed prompt and the original failure no longer occurs
...
**Check your work:**
- [ ] Current accuracy is recorded on all 20 tickets, not guessed
- [ ] Your improved classifier adds examples for the categories and edge cases that actually failed
- [ ] New accuracy is re-measured on the same 20 tickets and compared
- [ ] Each row in the change table has a "why" (the reasoning), not just a description of the change
...
**Check your work:**
- [ ] If you went for 95%+: you recorded where accuracy plateaued and why
- [ ] If you tested the minimum-example count: you have a small dataset and a recorded result
- [ ] If you built the urgency extractor: you included examples for High/Medium/Low and ran it
